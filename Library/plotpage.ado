*Writing titles to textfile
    ...
    capture program drop plotpage
    program plotpage
	    syntax, graph(string) tags(string) 
	...
	*Write the title rows defined above
	...
	file write  	`webpage' ///
		...
		`"  <meta name="keywords" content="`tags'" />"' _n ///
		`""' _n ///
		...
		`"        jQuery("#contain").load("https://raw.githubusercontent.com/worldbank/stata-visual-library/develop/docs/header.html");"' _n ///
		"    });" _n ///
		`"  </script>"' _n ///
		`""' _n ///
		`""' _n ///
		...
		`"<script type="text/javascript">"' _n ///
		"  jQuery(document).ready(function(){" _n ///
		`"	  jQuery("#stata").load("https://raw.githubusercontent.com/worldbank/stata-visual-library/develop/Library/`graph'.html");"' _n ///
		"  });" _n ///
		`"</script>"' _n ///
		`""' _n ///
		`""' _n ///
	...
	copy "`htmlfile'" "${GH}/docs/`graph'.html", replace
