* Figure: horizontal stack bar plot
    ...
    qui foreach var of varlist ///
        ...
        local ++x
        local theLabel : var label `var' // get variable label to use as plot label
        local theLabels `" `theLabels' `x' "`theLabel'" "' // [`mean'%]

        cap mat drop theResult // drop the result matrix in case it is still stored from a previous run of the code
		...
		* regress each variable on the independent variables, one by one, to record the variance explained by each
        ...
        mean `var' [pweight = weight_city] // calculate weighted mean
        mat a = e(b)
        local mean = a[1,1]
        mat theResult = nullmat(theResult) , [`mean']

        mat theResults = nullmat(theResults) \ theResult
        matlist theResults // eye check matrix
