* Figure: 10-25-50-75-90 percentile box plot
    ...
    global  graph_opts1 bgcolor(white) graphregion(color(white)) legend(region(lc(none) fc(none))) /// set all graph options in a global variable so they can be easily reused
    ...
    * Graph (10-25-50-75-90 pctiles)
    ...
    collapse (p10) p10=competence_mle (p25) p25=competence_mle (p50) p50=competence_mle ///
            ...
            , ///
              by(provider_cadre country) // calculate percentiles of the outcome variable by country and provider type

    reshape long p, i(provider_cadre country) // reshape the data so each percentile is in one row
	...
	* Replace country names with labels we want to show up in graph
    ...
    graph   box p /// box plot of percentiles
            ,  ///
            over(provider_cadre) over(country) /// plot over country and provider type
            hor /// display boxplot horizontally
