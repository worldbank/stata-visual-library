...
*/
  ...
  global  graph_opts title(, justification(left) color(black) span pos(17)) ///
      graphregion(color(white)) ylab(,angle(0) nogrid notick) xscale(noline) ///
      yscale(noline) yline(0 , lc(black)) ///
  ...
  twoway    (kdensity score if bach == 0 , lp(dash) lc(maroon) yaxis(2)) ///
      ...
      (histogram score if bach == 0 , freq w(.1) recast(scatter) ///
          msize(small) mc(maroon)) ///
      (histogram score if bach == 1 , freq w(.1) recast(scatter) ///
          msize(small) mc(navy)) ///
    , ///
    ...
    xtit("Knowledge Score {&rarr}") xlab(0(1)7) yline(10 20 30 , ///
	    lc(gs12) lp(dot)) xsize(7) ///
