*Figure: Scatter plot with fitted line and confidence interval
    ...
    * Graph
    twoway ///
        (lfitci jobs_scarce_code avg_growth ) /// linear prediction plot; y = jobs_scare_code, with CI
        ...
        (scatter jobs_scarce_code avg_growth if continent == "Europe", mcolor(ebblue ) m(T) ) ///   Adding scatter plots for each individual region
      ...
      ,  ///
        xlabel(-5(5)15)         ///  define x axis labels: (start(increment-by)end)
        ...
        ring(0) position(4)) /// edit legend position so it appears inside plot 
